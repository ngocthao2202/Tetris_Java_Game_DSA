	...
	public void update(){
		...
		if(currentMino.active == false) {
			...
			if(currentMino.b[0].x == MINO_START_X && currentMino.b[0].y == MINO_START_Y) {
				// this means the currentMino immediately collided a block and coundn't move at all
				// so it's xy are the same with the nextMino's
				gameOver = true;
				GamePanel.music.stop();
				GamePanel.se.play(2, true);
			}
		...
		while(x < right_x && y < bottom_y) {
			...
			for(int i = 0; i < staticBlocks.size(); i++) {
				if(staticBlocks.get(i).x == x && staticBlocks.get(i).y == y) {
					// increase the count if there is s static block
					blockCount++;
				}
			}
			...
			if(x == right_x) {
				// if the blockCount hints 12, that means the current y line is all filled with blocks
